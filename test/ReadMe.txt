...
应用程序向导已为您创建了此 test 项目，作为编写动态链接库 (DLL) 的起点。
...
test.idl
    ...
    此文件将由 MIDL 编译器处理，用于生成：
        ...
        GUID 定义                                (test_i.c)
        类型库                                  (test.tlb)
        封送处理代码                                 （test_p.c 和 dlldata.c）

test.h
    此文件包含 test.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
testps.vcxproj
	...
	此过程生成 dlldata.c、test_i.c 和 test_p.c，这些是生成代理/存根 (stub) DLL 所必需的。
